...
# Đường dẫn lưu tài khoản người dùng (Sửa lỗi đường dẫn)
USER_DB = r'C:/Users/Admin/TEST/dapgt/dagpt/user/user_data.json'

# Kiểm tra nếu file JSON không tồn tại thì tạo file trống
...
# Giao diện Streamlit
st.title("🔗 Kết nối PostgreSQL, Chọn Database, Schema & Phân Tích với GPT")
                    ...
                    for table in selected_tables:
                        for chunk in stream_data_from_table(engine, selected_schema, table, chunk_size):
                            ...
                            if not st.session_state.loading_data:
                                break
                    st.success(f"Đã tải dữ liệu từ {len(selected_tables)} bảng thành công ({len(st.session_state.df)} dòng).")
                except Exception as e:
                    st.error(f"Lỗi khi tải dữ liệu: {e}")
                st.session_state.loading_data = False
            
            if st.session_state.loading_data:
                if st.button("Dừng tải dữ liệu", key="stop_loading_data"):
                    st.session_state.loading_data = False
            
            # Chỉ tạo agent nếu đã có dữ liệu
